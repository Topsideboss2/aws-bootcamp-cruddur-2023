...
## Topics
* [Map out DynamoDB Table](#map-out-dynamodb-table)
* [Configure DynamoDB Environment](#configure-dynamodb-environment)
* [Create DynamoDB Python & Bash Scripts](#create-dynamodb-python--bash-scripts)
* [Implement Pattern A](#implementing-pattern-a)
* [Implement Pattern B](#implementing-pattern-b)
* [Implement Pattern C](#implementing-pattern-c)
* [Implement Pattern D](#implementing-pattern-d)
* [Implement Pattern E](#implementing-pattern-e)

## Map out DynamoDB Table

During the session with organizer [Andrew Brown](https://twitter.com/andrewbrown?s=21&t=sp1w7YU-708sR_DHopS9gg) and guest instructor [Kirk Kirkconnell](https://twitter.com/NoSQLKnowHow), we mapped out our single-table DynamoDB layout and designed the several patterns we would implement in accessing the data in this table.

#### DynamoDB-General-Layout
![](../_docs/assets/DynamoDB-Layout.png)

#### Pattern-A
Listing message-groups
![](../_docs/assets/Pattern-A.png)

#### Pattern-B
Listing messages in an existing message-group
![](../_docs/assets/Pattern-B.png)

#### Pattern-C
Creating a new message in an existing message-group
![](../_docs/assets/Pattern-C.png)

#### Pattern-D
Creating a new message in a new message-group
![](../_docs/assets/Pattern-D.png)

#### Pattern-E
Updating a message-group using Dynamo-DB streams
![](../_docs/assets/Pattern-E.png)

## Configure DynamoDB Environment

In `requirements.txt` add the boto3 environment as `boto3` and run `pip3 install -r requirements.txt`

Configure `AWS_ENDPOINT_URL: "http://dynamodb-local:8000"` environment variables in `docker-compose.yml` under backend-flask.

Add your AWS cognito user-pool ID as an env variable
...
We will create a separate folder for DynamoDB scripting. `backend-flask/bin/ddb/`
Scripts added are:
1. drop
```shell
#! /usr/bin/bash
...
def get_user_uuids():
  ...
  users = db.query_array_json(sql,{
    'my_handle':  'Murithi777',
    'other_handle': 'Topsideboss2'
  })
  my_user    = next((item for item in users if item["handle"] == 'Murithi777'), None)
  other_user = next((item for item in users if item["handle"] == 'Topsideboss2'), None)
  results = {
    'my_user': my_user,
...
We also created a python script to fetch Cognito Users in `/backend-flask/bin/db/`
1. update_cognito_user_ids
```python
#!/usr/bin/env python3
